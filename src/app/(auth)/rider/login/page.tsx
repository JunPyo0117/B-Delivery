  ...
  return (
    <div className="flex min-h-dvh flex-col items-center justify-center bg-white px-6">
      <div className="mb-6 flex h-[120px] w-[120px] items-center justify-center rounded-3xl bg-[#2DB400]">
        <span className="text-[56px] font-bold text-white">🛵</span>
      </div>

      <h1 className="text-[28px] font-bold tracking-tight">배달기사 전용</h1>
      <p className="mt-2 text-sm text-gray-500">B-Delivery 배달기사 앱</p>

      <form
        action={async () => {
          "use server";
          await signIn("google", { redirectTo: "/rider" });
        }}
      ...
      {process.env.E2E_TESTING === "true" && (
        ...
        >
          ...
          <div className="flex gap-2">
            <input name="email" type="email" defaultValue="rider@bdelivery.com" className="flex-1 rounded-lg border px-3 py-2 text-sm" />
            <button type="submit" className="rounded-lg bg-[#2DB400] px-4 py-2 text-sm font-medium text-white">로그인</button>
          </div>
        </form>
      )}
