...
**1) 실시간 주문 현황 (콜 보드)**
- **대기 중 주문 (PENDING):** 신규 주문 카드 실시간 수신 — **알림음 + 화면 깜빡임** (배민 사장님 앱 스타일)
  - 카드: 주문번호, 메뉴 요약, 주문 금액, 접수 시간, **경과 시간** (실시간 카운터)
  - **10초 이내 접수 유도:** 경과 시간이 3분 초과 시 카드 빨간색 강조
- **조리 중 (COOKING):** 조리 중인 주문 리스트 — 각 주문별 예상 완료 시간 표시
- **배달 대기 (WAITING_RIDER):** 기사 매칭 중인 주문 리스트 — "기사 찾는 중..." 애니메이션 + 경과 시간
- **배달 중 (PICKED_UP):** 현재 배달 진행 중인 주문 — 기사 정보 + 예상 도착 시간
- **칸반 보드 레이아웃:** 대기 | 조리중 | 배달대기 | 배달중 — 4컬럼으로 한눈에 파악

**2) 오늘의 매출 요약**
- **실시간 매출:** 오늘 총 주문금액 (실시간 갱신)
- **주문 건수:** 오늘 접수된 주문 수 / 완료된 주문 수 / 취소된 주문 수
- **평균 주문 금액:** 오늘 완료 주문 기준
- **시간대별 주문 추이:** 막대 그래프 (00시~23시, 현재 시간 하이라이트)

**3) 인기 메뉴 TOP 5**
- 오늘 기준 가장 많이 주문된 메뉴 순위
- 메뉴명, 주문 수, 매출 기여도 (%)

**4) 리뷰 알림**
- 최근 24시간 신규 리뷰 목록 (미답변 리뷰 강조)
- 평균 별점 추이 (최근 7일)
- 클릭 시 리뷰 상세 → 바로 답글 작성 가능

**5) 영업 상태 토글**
- 영업 중 / 영업 종료 / 일시 중지 (주문 폭주 시)
