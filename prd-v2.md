...
- **음식점 위치 입력:**
  ...
  - 지도 핀으로 위치 확인 가능
- **메뉴 빠른 시작 (등록 마지막 단계) (NEW):**
  - 카테고리 선택 시 **기본 메뉴 템플릿 자동 제안** — "치킨집 기본 메뉴를 추가할까요?"
  - 3가지 옵션 중 선택:
    1. **카테고리 템플릿 적용** — 카테고리별 대표 메뉴 + 옵션 자동 생성 (가격만 입력)
    2. **다른 가게 메뉴 복사** — 같은 이름의 프랜차이즈 또는 같은 카테고리 인기 가게 메뉴 가져오기
    3. **처음부터 직접 등록** — 빈 메뉴에서 시작
  - 선택 후 메뉴 관리 페이지로 이동하여 가격/이미지 등 수정
...
- 메뉴 품절 처리 (ON/OFF)
- **메뉴 가져오기 (NEW):**
  - 메뉴 관리 페이지 상단 "메뉴 가져오기" 버튼
  - **카테고리 기본 템플릿:** 내 가게 카테고리의 대표 메뉴 + 옵션 일괄 추가 (가격 0원 → 사장이 입력)
  - **프랜차이즈 메뉴 복사:** 같은 가게 이름으로 검색 → 다른 지점의 메뉴 구조(메뉴명, 카테고리, 옵션 그룹/항목) 복사. 가격은 0원으로 초기화, 이미지는 복사 안 함.
  - **인기 가게 메뉴 참고:** 같은 카테고리 인기 음식점 리스트 → 선택한 가게의 메뉴 구조 복사
  - 복사 후 사장이 가격/이미지/불필요 메뉴를 수정/삭제

**메뉴 템플릿 데이터 (시스템 내장):**
- 카테고리별 대표 메뉴와 옵션을 시드 데이터(JSON)로 관리
- 예시 — 치킨 카테고리:
